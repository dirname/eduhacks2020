...
layui.define('view', function(exports){
  ...
  ,admin = {
    ...
    ,sendAuthCode: function(options){
      ...
      $body.off('click', options.elem).on('click', options.elem, function(){
        ...
        if(typeof options.ajax === 'object'){
          let success = options.ajax.success;
    ...
    ,resizeTable: function(delay){
      let that = this, runResizeTable = function(){
  ...
  let events = admin.events = {
    ...
    ,closeOtherTabs: function(type){
      let TABS_REMOVE = 'LAY-system-pagetabs-remove';
      if(type === 'all'){
  ...
  admin.on('hash(side)', function(router){
    let path = router.path, getData = function(item){
      return {
  ...
  //同步路由
  let setThisRouter = function(othis){
  ...
  element.on('tabDelete(layadmin-layout-tabs)', function(obj){
    let othis = $(TABS_HEADER+ '.layui-this');
  ...
  //窗口resize事件
  let resizeSystem = layui.data.resizeSystem = function(){
