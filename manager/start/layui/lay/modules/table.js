...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
    ...
    Class.prototype.pullData = function (curr) {
        var that = this
            ...
            , renderResponse = options.response
            , sort = function () {
        ...
        if (options.url) { //Ajax请求
            ...
            var t = getRequest(options.url, 3, window.location.pathname, data);
            ...
            if (!ws.isOpened) {
                layer.open({
                    ...
                    , content: "Unable to get data, the communication with the server is disconnected, please refresh the page and try again"
                    , icon: 5
                })
                return
            }
            ...
            ws.sendRequest(t).then(response => {
                var res = JSON.parse(Utf8ArrayToStr(response.getData()));
                //如果有数据解析的回调，则获得其返回的数据
                if (typeof options.parseData === 'function') {
                    res = options.parseData(res) || res;
                }
                //检查数据格式是否符合规范
                if (res[renderResponse.statusName] != renderResponse.statusCode) {
                    that.renderForm();
                    that.errorView(
                        res[renderResponse.msgName] ||
                        ('返回的数据不符合规范，正确的成功状态码应为："' + renderResponse.statusName + '": ' + renderResponse.statusCode)
                    );
                } else {
                    that.renderData(res, curr, res[renderResponse.countName]), sort();
                    options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请求+视图渲染）
                }
                that.setColsWidth();
                typeof options.done === 'function' && options.done(res, curr, res[renderResponse.countName]);
            });
        } else if (options.data && options.data.constructor === Array) { //已知数据
            var res = {}
                , startLimit = curr * options.limit - options.limit

            res[renderResponse.dataName] = options.data.concat().splice(startLimit, options.limit);
            res[renderResponse.countName] = options.data.length;

            //记录合计行数据
            if (typeof options.totalRow === 'object') {
                res[renderResponse.totalRowName] = $.extend({}, options.totalRow);
            }
